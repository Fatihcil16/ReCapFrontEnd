...
export class CartComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.cartService.list().length == 0) {
      this.toastrService.info('Your Cart is Empty, Forwarding ...');
  ...
  checkCarReturnDate(): boolean {
    if (this.carDetailReturnDate != undefined) {
      ...
      if (date1.getFullYear() > date2.getFullYear()) {
        this.toastrService.error('The vehicle has already been rented on this date!');
        return false;
      ...
      if (this.now.getFullYear() > this.model.year) {
        this.toastrService.error('Cannot buy a Vehicle for the past date');
        return false;
