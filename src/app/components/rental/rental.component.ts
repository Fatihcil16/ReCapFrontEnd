...
export class RentalComponent implements OnInit {
  ...
  setRentingCar() {
    ...
    if (returnDate < rentDate || returnDate.getDate() == rentDate.getDate()) {
      return this.toastrService.warning('Select Another Return Date', 'Warning');
    }
  ...
  checkCarRentable() {
    ...
    this.rentalService.getRentalsByCarId(this.carId).subscribe((responseSuccess) => {
      ...
      if (new Date(this.rental.rentDate) < returnDate) {
        return this.toastrService.warning('You cannot rent this vehicle between these dates.', 'Warning');
      }
